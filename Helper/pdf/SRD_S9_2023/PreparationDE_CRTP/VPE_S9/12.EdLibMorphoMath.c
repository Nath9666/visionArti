...
int Erosion3(EdIMAGE *image, EdIMAGE *imres)
{
  ...
  int	i,j;                             
  int flagEros; /* example of pixel read and write ... */
           ...
           POINT_X(point)++)
  {
    ...
    if (flagEros == TRUE)
      PIXEL(imres, point) = 0; // Erosion
    else
      PIXEL(imres, point) = PIXEL(image, point); // Recopie
...
// ==================================================================
int Dilatation3(EdIMAGE *image, EdIMAGE *imres)
{
  // current and neighbour (voisin in French) point creation 
  EdPOINT	*point = NULL, *pointv = NULL;
  // index of lines and columns of the 3X3 neighbourhood
  int	i,j;                             
  int flagDilat; /* example of pixel read and write ... */
           ...
           POINT_X(point)++)
  {
    ...
    if (flagDilat == TRUE)
      PIXEL(imres, point) = 255; // Dilatation
    else
      PIXEL(imres, point) = PIXEL(image, point); // Recopie
