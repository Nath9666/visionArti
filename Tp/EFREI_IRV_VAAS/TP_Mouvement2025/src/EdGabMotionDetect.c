...
#include "EdLibGabMotionDetect.h"


int main(int argc, char **argv)
{
  EdIMAGE	    *imCur = NULL,*imMin = NULL, *imMax = NULL, *imRes = NULL;
  int	        nlig = 0, ncol = 0;
  int           NumRef, Num;
  ...
  if(argc != 2)
  {
    fprintf(stderr,"USAGE :  MotionDetect image  \n");
  ...
  if (crea_IMAGE(imMin) == NULL)	/* creation of Image Header  */
  {
    fprintf(stderr,"Error of Memory Allocation  \n");
    exit (0);
  }
  ret = Creation_Image (imMin, nlig, ncol, prof); // Image Data

   if (crea_IMAGE(imMax) == NULL)	/* creation of Image Header  */
  {
    fprintf(stderr,"Error of Memory Allocation  \n");
    exit (0);
  }
  ret = Creation_Image (imMax, nlig, ncol, prof); // Image Data
  ...
  ret = InitGabarit(imMin, imMax, imCur);

  //Boucle d'élaboration du gabarit 
    while (Num < 50)
  {
      Num++;
      ...
      ret = UpDateGab(imMin, imCur, imCur);
  ...
  while (1)
  {
      ...
      /* --- Detection du Mouvement --- */
      ret = GabMotionDetect(imMin, imMax, imCur, imRes);
